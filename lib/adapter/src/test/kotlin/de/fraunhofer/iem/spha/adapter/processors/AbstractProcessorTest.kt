...
/**
 * Abstract base test class for processor tests.
 * Provides common test patterns for testing tool processors.
 *
 * Subclasses should implement the abstract methods to provide processor-specific
 * test data and validation logic.
 */
abstract class AbstractProcessorTest<T : Origin> {

    /**
     * Returns a list of invalid JSON strings that should cause the processor to return null.
     * These are strings that don't match the processor's expected format.
     */
    abstract val invalidInputs: List<String>

    /**
     * Returns a list of valid test resource file paths (relative to src/test/resources).
     * Each file should contain valid input for the processor.
     */
    abstract val validTestResourceFiles: List<String>

    /**
     * Returns whether the processor supports empty input (returns a valid result for empty/blank content).
     * Default is false. Override to return true if the processor handles empty input specially.
     */
    open val supportsEmptyInput: Boolean
        get() = false

    /**
     * Returns the processor instance to be tested.
     * Subclasses must implement this to provide the specific processor.
     */
    internal abstract fun getProcessor(): ToolProcessor

    /**
     * Processes the given content string and returns the result.
     * This is a non-virtual method that delegates to the processor returned by [getProcessor].
     * @param content The JSON content to process.
     * @return The AdapterResult if processing succeeds, or null if the content doesn't match the processor's format.
     */
    ...
    /**
     * Validates the result from processing a valid input.
     * Subclasses can override this to add processor-specific validation.
    ...
    open fun validateResult(result: AdapterResult<T>, resourceFile: String) {
        // Default implementation is empty - base class already validates Success<T> in testValidResourceFiles
    }
    ...
    /**
     * Validates the result from processing empty input.
     * Only called if [getSupportsEmptyInput] returns true.
    ...
    open fun validateEmptyInputResult(result: AdapterResult<T>) {
        ...
        if (supportsEmptyInput)
            fail { "Override this method to validate empty input results for processors that support empty input." }
    }
    ...
    fun testInvalidInputReturnsNull() {
        invalidInputs.forEach { invalidInput ->
            assertNull(
                ...
                "Expected null for invalid input: $invalidInput"
            )
        }
    }
    ...
    fun testEmptyInput() {
        ...
        if (supportsEmptyInput) {
            ...
            result.transformationResults.forEach { 
                assertTrue(it is TransformationResult.Success<T>, "Expected all results to be Success<T>")
            }
    ...
    fun testValidResourceFiles() {
        ...
        resourceFiles.forEach { resourceFile ->
            ...
            val successes = result.transformationResults.filterIsInstance<TransformationResult.Success<T>>()
            assertTrue(successes.isNotEmpty(), "Expected at least one Success<T> result for $resourceFile")
