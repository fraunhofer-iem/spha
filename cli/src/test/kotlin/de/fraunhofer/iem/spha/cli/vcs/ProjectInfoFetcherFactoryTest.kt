...
class ProjectInfoFetcherFactoryTest {
    ...
    fun `createFetcher with string creates correct fetcher`(
        typeString: String,
    ...
    fun `detectRepositoryType detects correct type from URL`(
        url: String,
        expectedType: RepositoryType,
    ) {
    ...
    fun `detectRepositoryType throws for unknown host`() {
        val exception =
            assertFailsWith<IllegalArgumentException> {
                ProjectInfoFetcherFactory.detectRepositoryType(
    ...
    fun `createFetcherFromUrl creates correct fetcher for URL`(
        url: String,
        expectedClass: kotlin.reflect.KClass<*>,
    ) {
        val fetcher = ProjectInfoFetcherFactory.createFetcherFromUrl(url)
        fetcher.use { assertTrue(expectedClass.isInstance(it)) }
    }
    ...
    fun `createFetcherFromUrl throws for unknown host`() {
        val exception =
            assertFailsWith<IllegalArgumentException> {
                ProjectInfoFetcherFactory.createFetcherFromUrl(
                    "https://unknown-host.com/owner/repo"
                )
            }
        assertEquals(exception.message?.contains("Unable to determine repository type"), true)
