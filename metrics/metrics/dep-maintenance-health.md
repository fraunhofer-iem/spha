...
Measures the active maintenance health of an open source dependency by combining three independently automatable signals drawn from the CHAOSS project's viability and community health metrics framework. Together these signals answer the key question: _can we rely on this dependency continuing to receive security fixes, bug fixes, and active development?_

The three signals are:

- **Release Frequency** _(CHAOSS metric)_ — how regularly the project ships releases, including patch releases that carry security fixes
- **Contributor Absence Factor** _(CHAOSS metric)_ — the smallest number of contributors responsible for 50% of commits; a low value indicates a project is at risk of stalling if a key contributor leaves (the XZ and OpenSSL incidents are canonical examples of this risk)
...
- **Score direction:** `dep-maintenance-health` is a _health_ score — higher is better. It is **inverted** (`1.0 - score`) when used as a risk input in `dep-risk-score`, so that a healthy dependency does not inflate risk.
